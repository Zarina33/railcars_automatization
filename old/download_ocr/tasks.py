...
def ocr_number(image):
    """Function to perform OCR on a specific region of the image."""
    cropped_image = image[120:360, :]

    # Convert the image to grayscale
    gray_image = cv2.cvtColor(cropped_image, cv2.COLOR_BGR2GRAY)

    # Invert the image colors to make white text on a black background (optional)
    inverted_image = cv2.bitwise_not(gray_image)

    # Extract only numbers from the image
    results = reader.readtext(inverted_image, detail=0, allowlist='0123456789')
    return results
...
def process_downloaded_images(image_folder):
    """Функция для обработки загруженных изображений с использованием OCR."""
    reader = easyocr.Reader(['en'])  # Инициализация внутри задачи
    text_folder = os.path.join(LOCAL_BASE_FOLDER, 'results')

    # Создаем папку 'results', если она не существует
    os.makedirs(text_folder, exist_ok=True)

    # Обрабатываем каждое изображение в папке 'images'
